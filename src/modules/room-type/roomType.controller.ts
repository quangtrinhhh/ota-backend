...
export class RoomTypeController {
  ...
  async create(@Body() createRoomTypeDto: CreateRoomTypeDto) {
    try {
      ...
      const createRoomType = await this.roomTypeService.create(createRoomTypeDto);
      return new ResponData(createRoomType, HttpStatus.SUCCESS, HttpMessage.SUCCESS);
    } catch (error) {
      console.error(error); 
  ...
  async findAllRoomType() {
    try {
      ...
      return new ResponData(findAllRoomType, HttpStatus.SUCCESS, HttpMessage.SUCCESS);
    } catch (error) {
      console.error(error); 
      return new ResponData(null, HttpStatus.ERROR, HttpMessage.ERROR);
    }
  ...
  async deleteRoomType(@Param('id') id:number):Promise<ResponData<string>>{
    try {
      return new ResponData<string>(await this.roomTypeService.deleteRoomType(id), HttpStatus.SUCCESS, HttpMessage.SUCCESS);
    } catch (error) {
  ...
  async updateRoomType(
    ...
    try {
      ...
      return new ResponData(updateRoomType, HttpStatus.SUCCESS, HttpMessage.SUCCESS);
    } catch (error) {
      console.error(error);
      return new ResponData(null, HttpStatus.ERROR, HttpMessage.ERROR);
    }
