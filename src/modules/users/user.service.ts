...
export class UserService {
  ...
  async getUsersByHotelId(id: number) {
    ...
    if (!user) {
      // Trả về thông báo lỗi khi không tìm thấy người dùng
      throw new Error('User not found');
    }

    const hotelId = user.hotel_id; // Lấy `hotel_id` của user

    // Lấy danh sách tất cả user có `hotel_id` giống với hotelId
    const users = await this.userRepository.find({
      select: ['id', 'user_name'], // Chỉ chọn trường `id` và `user_name`
