...
export class StatisticsService {
    ...
    async getRoomCapacityAndOccupancy(hotel_id: number): Promise<any> {
        ...
        const roomInUser = await this.roomRepository.find({
            where: {
                hotel_id,
                status: Not(ROOM_STATUS.EMPTY),
            }
        });
        const roomCapacity: string = ((roomInUser.length / room.length) * 100).toFixed(1) + '%';
        const totalPeople = {
            children: 0,
            adults: 0,
        };

        const bookings = await this.bookingRepository.find({ where: { status: 'CheckedIn' } });
